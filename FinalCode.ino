//zalaczamy potrzebne biblioteki
#include <SPI.h>
#include <Wire.h>
...
//definiujemy piny na ktorych podlaczylismy urzadzenia
...
//ogolne zmienne potrzebne do funkcjonowania
long duration;
int distance;
int timespressed = 0;
...
//max czas miedzy kliknieciami paniki
#define maxtimebetweenpresses 10000 // 10 seconds

//inicjalizacja sensora wilgoci
DHT11 dht11(humidityPin);

void setup() {
  Serial.begin(9600);//komunikacja z lapkiem
  ...
  if (!IMU.begin()) {
    ...
    //ustawienia wsteplne oleda
    display.setTextSize(1);
    display.clearDisplay();

   if (WiFi.status() == WL_NO_MODULE) {
    display.println("Communication with WiFi module failed!");
    display.display();//sprawdzenie statusu polaczena wifi
    while (true);
  }

  //powiadomienie na ekranie o polaczeniu
...
//funkcja ktora ma za zadanie zlapac wcisniecia guzika paniki i inicjowac sekwencje paniki i jej opuszczenia
void InitiatePanic() {
  if (digitalRead(panicPin)) {
    ...
    delay(100);
    //guzik klikniety odnotowano kiedy i kotry raz z rzedu
  }
  if (timespressed >= 3) {
    Serial.println("Panic!!!");
    delay(200);
    LeavePanic();
    ////jesli 3 raz z rzedu kliknelismy guzik to jestesmy w panice zaczynamy szukac warunkow by ja opuscic
  }
  if (millis() - lastpressed >= maxtimebetweenpresses) {
    timespressed = 0;
    //resetujemy ilosc klikniec jesli czekalismy za dlugo
  }
  delay(200);
}

//funkcja opusczania stanu paniki
void LeavePanic() {
  while (1) {
    //opuscimy panike dopiero gdy warunki sie znormalizuja lub wcisniemy guzik 
    digitalWrite(diodePin, HIGH);
    digitalWrite(buzzerPin, HIGH);
    display.clearDisplay();
    display.setCursor(2, 2);  // Position at the top-left corner
    display.setTextSize(3);
    display.print("DANGER!");//powiadom uzytkownika na wyswitlaczu o niebezpieczenstwie
    display.display();
    delay(100);
    ...
    if (timespressed <= 0) {
      ...
      break;
      //panika sie konczy jesli wcisniemy guzik w odpowiedniej sekwencji
    }
    if (digitalRead(panicPin)) {
      timespressed--;
      lastpressed = millis();
      Serial.println("pressed");
      delay(200);
      //funkcja do zliczania konsekwentnych klikniec
    }
    if (millis() - lastpressed >= maxtimebetweenpresses) {
      timespressed = 3;
    //jesli minelo za duzo czasu wyzeruj ilosc kliniec
    }
    digitalWrite(diodePin, LOW);
...
//funkcja do zbierania danych z czujnika wilgoci i temperatury
void TempHumidityG() {
  float gs = getG();
  int temperature = 0;
  int humidity = 0;
  ...
  if (humidity >= 80 || temperature >= 50 || gs > 5) {
    //inicjalizujemy panike jesli warunki sa nie do zycia
    LeavePanic();
  }
}

//liczymy dlugosc wektora grawitacji dzialajacego na nas
float getG() {
  float x, y, z;
...
//liczy odleglosc od przedmiotu na ktory wskazujemy laserem za pomoca czujnika ultradwieku
void GetDistance()
{
 while(1){
    ...
    display.print(distance);
    display.display();
    digitalWrite (laserPin, HIGH); // fire the laser!!!
    delay(500);
    if(!digitalRead(laserButtonPin))
    //przy kliknieciu guzika zmieniamy tryb na kolejny
    {
      indicator++;
      display.clearDisplay();
      display.display();
...
//funkcja zamienia sile sygnalu w dBm na odleglosc od zrodla w cm
float rssiToDistance(float rssi) {
  //algo to calculate distance from signal strength
  float distance = pow(10, (27 - rssi) / 100.0)*10;
  //matematycznie dopasowana funkcja ktora dala nam najlepsze wyniki
  return distance;
}

//odleglosc od zrodla sygnalu
void WifiDistance()
{
   display.clearDisplay();
  display.display();
  while(1){
    //zbieramy od modulu wifi dane o rssi
      display.clearDisplay();
    int rssi = WiFi.RSSI();
  ...
  delay(100);
    if(!digitalRead(laserButtonPin))
    {
      //po kliknieciu guzika zmieniamy tryb na kolejny
      indicator++;
      display.clearDisplay();
      display.display();
      delay(1000);
...
void GetPulse()
{
  ...
  {
    sum+= analogRead(A0);
    //srednia 100 pomiarow by zwiekszyc dokladnosc
  }
  pulse = sum/100.0;
  Serial.println(pulse);
  //ze wzgledu na jakosc czujnika i jego specyficzne wymagania(kompletna izolacja od swiatla) staramy sie znormalizowac dane 
  pulse = abs(pulse);
...
void loop() {
  ...
  //w zaleznosci od wartosci indykatora wlaczamy konkretne tryby
